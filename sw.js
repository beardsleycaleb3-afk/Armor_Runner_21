...
// v3.1 - FIXED: Caches ALL assets automatically (backgrounds + defenders)
// ============================================================================
...
self.addEventListener('fetch', (event) => {
  ...
  // ALL Images in assets/ folder - Cache-first with long expiry
  // This automatically caches backgrounds, defenders, icons, etc.
  if (path.includes('/assets/') && path.match(/.(jpg|jpeg|png|gif|webp|svg)$/)) {
    return event.respondWith(
      caches.match(request).then((response) => {
        return response || fetch(request)
          .then((response) => {
...
console.log('[Service Worker] Auto-caches ALL assets/ folder images (backgrounds, defenders, etc.)');
